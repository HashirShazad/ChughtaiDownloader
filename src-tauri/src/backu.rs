    ...
    for table in html.select(&table_selector){ // get all tables from html
        ...
        for row in table.select(&row_selector) { // get all rows from table
            ...
            for href in row.select(&href_selector) { // Get all links from row
                ...
                let img_selector = Selector::parse("img").unwrap(); // helps in selecting images from rows

                for img in row.select(&img_selector) {// get all images from row

                    // these are constants just used for knowing the file type and what to perform
                        ...
                        if alt == is_parent_directory || alt == is_icon{
                            ...
                            if alt == is_directory {
                                ...
                                if download_imgs{
                                    ...
                                    // download_file_from_url_with_folder(&href_link.as_str(), &folder_to_download).await.map_err(|e| TauriError::from(e.into()))?;
                                }
                                else{
